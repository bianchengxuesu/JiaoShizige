 ...
 <div class="navbg">
  ...
  <div class="navbar-collapse collapse">
   <ul class="nav navbar-nav mainNav">
    <li><a href="https://aacsb.ustc.edu.cn/tjut/Department.asp#" class="dropdown-toggle" data-toggle="dropdown">我的信息<b class="caret"></b></a>
    ...
    <li><a href="https://aacsb.ustc.edu.cn/tjut/Department.asp#" class="dropdown-toggle" data-toggle="dropdown">系统管理<b class="caret"></b></a>
     <ul class="dropdown-menu">
	...
	<li><a href="https://aacsb.ustc.edu.cn/tjut/Department.asp#" class="dropdown-toggle" data-toggle="dropdown">标准数据<b class="caret"></b></a>
	 <ul class="dropdown-menu">
	...
	<li><a href="https://aacsb.ustc.edu.cn/tjut/Department.asp#" class="dropdown-toggle" data-toggle="dropdown">智力贡献<b class="caret"></b></a>
	 <ul class="dropdown-menu">
    ...
    <li><a href="https://aacsb.ustc.edu.cn/tjut/Department.asp#" class="dropdown-toggle" data-toggle="dropdown">表格输出<b class="caret"></b></a>
	 <ul class="dropdown-menu">
    ...
    <li><a href="https://aacsb.ustc.edu.cn/tjut/Department.asp#" class="dropdown-toggle" data-toggle="dropdown">信息检索<b class="caret"></b></a>
	 <ul class="dropdown-menu">
...
<nav class="navbar navbar-default">
    ...
    <div class="navbar-header" style="text-align:right;">
	  <button type="button" class="navbar-toggle collapsed navbar-toggle_col" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1" aria-expanded="false">
        ...
        <span class="navbar-brand"><b>部门信息</b></span>
    </div>
    <div>
...
<ul class="nav navbar-nav navbar-left">
 <li class="dropdown">
  <a href="https://aacsb.ustc.edu.cn/tjut/Department.asp#" class="dropdown-toggle font16" data-toggle="dropdown">栏目导航<b class="caret"></b>
  </a>
...
--><ul class="nav navbar-nav navbar-right">
 ...
 <li class="active font15"><a href="department">部门</a></li>
...
<form name="form1" action="https://aacsb.ustc.edu.cn/tjut/Department.asp" method="post">
 ...
 <button type="submit" class="btn btn-default"><span style="font-size:12px;"><span class="glyphicon glyphicon-search"></span></span> 搜索</button>
  ...
  <span class="pull-right">
   ...
   <button type="submit" name="action" form="deleteDepartment" onclick="return check();" class="btn btn-danger mt-3 mb-3 mr-5 font16"><span class="glyphicon glyphicon-trash"></span> 删除</button>
   <a class="btn btn-success mt-3 mb-3 font16" href="departmentadd"><span class="glyphicon glyphicon-plus"></span> 添加</a>
...
<div class="container">
 <div class="mt-10" style="min-height:130px;">
   <form id="deleteDepartment" action="deleteDepartment.do" method="post">
    <table class="table">
     <thead>
      <tr class="active">
       <th class="co-w35">sel.</th>
	   <th class="co-w230">部门名称</th>
     ...
     <c:forEach items="${ps.list}" var="L">
      <tr onmouseover="this.bgColor=&#39;f9f9f9&#39;;this.style.cursor=&#39;hand&#39;" onmouseout="this.bgColor=&#39;ffffff&#39;" data-toggle="tooltip" data-placement="top" title="" data-original-title="第1行">
	   <td class="co-w35"><input id="delid" name="delid" type="checkbox" class="bigbox" value="${L.deid}"></td>
       <td class="co-w230">${L.chineseName}</td>
	   <td class="co-hide-900">${L.englishName}</td>
	   <td class="co-w80 co-hide-640">${L.peopleNum}</td>
	   <td class="co-w60 text-center"><a class="mr-5" href="javascript:void(0)" name="Move.asp?run=ok&amp;t=Department&amp;ufiles=Department&amp;id=DepID&amp;at_ID=30&amp;OrderBy=1&amp;action=Up&amp;pa=1&amp;h=" onclick="getvalue(this.name)"><span class="glyphicon glyphicon-arrow-up"></span></a> <a href="javascript:void(0)" name="Move.asp?run=ok&amp;t=Department&amp;ufiles=Department&amp;id=DepID&amp;at_ID=30&amp;OrderBy=1&amp;action=Down&amp;pa=1&amp;h=" onclick="getvalue(this.name)"><span class="glyphicon glyphicon-arrow-down"></span></a></td>
	   <td class="co-w50 text-center"><a class="btn btn-default btn-bin" onclick="TestBlack(&#39;set-${L.deid}&#39;);"><span class="glyphicon glyphicon-cog" title="点击展开进行查看和管理"></span></a></td>
      </tr>
	  <tr id="set-${L.deid}" style="display:none;">
       <td colspan="20" style="padding:0">
	    <div class="pull-right">
		 <div class="hidden">${L.chineseName}</div>
		 <div class="hidden">${L.englishName}</div>
		 <div class="hidden"></div>
<!--	 <a class="btn btn-default mt-10 mb-10 mr-5" name="DepSpLink.asp?DepID=30&amp;h=" onclick="getvalue(this.name) ">专业方向</a> -->
		 <a class="btn btn-default mt-10 mb-10 mr-5" name="managelink?deid=${L.deid}&amp;h=" onclick="getvalue(this.name) ">人员管理</a>
		 
         <a class="btn btn-default mt-10 mb-10 mr-5" name="DepartmentEdit.asp?DepID=30&amp;page=1&amp;h=" onclick="getvalue(this.name) ">编辑</a>
	 ...
	 <input name="aurl" type="hidden" value="http://aacsb.ustc.edu.cn/tjut/Department.asp">
	</form>
 </div>
</div>
...
<ul class='pagination'>
	...
	<c:if test="${ps.pageNum != 1}">
		<li><a href="department?page=${ps.pageNum-1}" aria-label="Previous"><span aria-hidden='true'>&laquo;</span></a></li>
	</c:if>
	<c:forEach begin="1" end="${ps.pages}" var="pageNumber">
		<c:if test="${pageNumber == ps.pageNum}">
			<li class="active"><a href="department?page=${pageNumber}">${pageNumber}</a></li>
		</c:if>
		<c:if test="${pageNumber != ps.pageNum}">
			<li><a href="department?page=${pageNumber}">${pageNumber}</a></li>
		</c:if>
	</c:forEach>
	...
	<c:if test="${ps.pageNum != ps.lastPage}">
		<li><a href="department?page=${ps.pageNum+1}" aria-label="Next"><span aria-hidden='true'>&raquo;</span></a></li>
	</c:if>
	</c:if>
</ul>
</nav>
